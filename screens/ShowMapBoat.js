...
//////////////////////////////////////////////
  ...
  fetchbusActiveData = async () => {
    ...
    let lineQuery = []
    if(searchTxt == ""){ 
    const response = await fetch(`${SettingApp}/getVehicleActive?Vehicle_Type='Boat'`)
    ...
    await this.setState({ BusActive})
      ...
      const response = await fetch(`${SettingApp}/getVehicleActiveQuery?Vehicle_Type='Boat'&lineQuery=${lineQuery}`)
      const BusActive = await response.json()
      await this.setState({ BusActive})
    }
  }
        ...
        >
          ...
          {this.state.BusActive.map((marker, key) => {
            ...
            return (
              ...
              >
                <BoatActive textLine={marker.busline_code}  />
              </Marker>
            )
          })}
           <Marker
